...
def script() -> None:
    ...
    if args.plot:
        ...
        if args.divide_radial: 
            bins_short = np.zeros((max_segs, bins.shape[1]), dtype=bins.dtype)
        ...
        if args.rv:
            ...
            LV_means = all_means[:17]

            ext_bullseye = utils.bullseye_plot_ext(all_means,
                                                title='Helical Angle ',
                                                cmap=helix_angle_cmap
                                                )
            ext_pdf = outdir / "bullseye_ext.pdf"
            LV_bullseye.savefig(ext_pdf, bbox_inches="tight")
            plt.close(ext_bullseye)

            LV_bullseye = utils.bullseye_plot(LV_means,
                                            title='LV Helical Angle',
                                            cmap=helix_angle_cmap
                                            )
            out_pdf = outdir / "bullseye_LV.pdf"
            LV_bullseye.savefig(out_pdf, bbox_inches="tight")
            plt.close(LV_bullseye)
