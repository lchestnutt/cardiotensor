...
def find_consecutive_points(start_point, vector_field, num_steps=4, segment_lenth=10, angle_threshold=60):
    ...
    for _ in range(num_steps):
        ...
        if 0 <= z < vector_field.shape[1] and 0 <= y < vector_field.shape[2] and 0 <= x < vector_field.shape[3]:
            ...
            if (
                0 <= next_point[0] < vector_field.shape[1] and
                0 <= next_point[1] < vector_field.shape[2] and
                0 <= next_point[2] < vector_field.shape[3]
            ):
...
def write_am_file(consecutive_points_list, HA_angle, z_angle, file_path="output.am"):
    ...
    with open(file_path, 'w') as f:
        ...
        f.write("define VERTEX {}\n".format(len(consecutive_points_list) * 2))  # Two vertices per line
        f.write("define EDGE {}\n".format(len(consecutive_points_list)))  # One edge per line segment
        f.write("define POINT {}\n".format(N_point))  # One edge per line segment
        f.write("\nParameters {\n")
...
def amira_writer(conf_file_path,start_index=None,end_index=None, bin_factor=None):
    ...
    for point_list in consecutive_points_list:
        for point in point_list:
            HA_angle.append(float(HA_volume[point[0]-start_index,point[1],point[2]]))
    ...
    write_am_file(consecutive_points_list, HA_angle, z_angle, file_path=OUTPUT_DIR / 'output.am')
...
def scale_points(consecutive_points, pixel_size):
    ...
    for point_list in consecutive_points:
        ...
        scaled_point_list = [tuple(coord * pixel_size for coord in point) for point in point_list]
