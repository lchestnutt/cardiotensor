...
from cardiotensor.utils  import convert_to_8bit
...
def calculate_structure_tensor(volume, SIGMA, RHO, device='', block_size='', use_gpu=False):
    """
...
def write_vector_field(vector_field_slice, start_index, output_dir, slice_idx):
    ...
    np.save(f"{output_dir}/eigen_vec/eigen_vec_{(start_index + slice_idx):06d}.npy", vector_field_slice)
